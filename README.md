...
本项目提供了一个训练框架，支持在图像分类任务中使用多种预训练模型进行训练。脚本支持训练多个模型，自动保存每个模型的最佳表现，并绘制多模型训练的对比图 。

## 数据集分割 (data_split)

### 介绍

为了确保模型训练和验证的公正性，我们需要将数据集划分为训练集（train）和验证集（val）。此部分代码实现了数据集的自动分割，将指定路径下的数据集按照设定的比例划分为训练集和验证集，方便进行后续的模型训练和评估。
